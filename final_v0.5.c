...
//char *path ="/home/TU/tug31545/testCHDIR";
...
int main(int argc, char *argv[], char * envp[])
{
    ...
    int pipee = 0;
    int bground =0; // background operation identifier
    ...
    if (argc >= 2 )  // batch file mode
    {
        char temp[50] = "";
        int counter = 0;
        FILE *open = fopen("batchMODE.txt","r");
        ...
        while(fgets(temp,50,open) != NULL) // store line in temp
        {
            //printf("%s\n",temp);
            char* holder = strtok(temp," "); //holder in 1 arg in temp line

            while(holder != NULL)
            {
                //char* holder = strtok(temp," ");
                batchModeArgs[counter] = holder; // place arg
                //printf("bma %d set to %s\n",counter,batchModeArgs[counter]);
                counter=counter+1;
                holder = strtok(NULL," ");
            }
            batchModeArgs[counter] == '\0'; // seperate diff command line input by space		   	  }
            counter=counter+1;
        }
        int view=0; // this should print out what i had in the above arry but the values change for some reason
        while(batchModeArgs[view] != "\n")
        {
            evaluation[view]=batchModeArgs[view];
...
//------------------------------------------------------------------
        if (argc >= 2 )  // turn batchModeArgs to evaluation argumets so they can be prossed
        {

            // set up evaluation from batchModeArgs
        }
        else
        {
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------
//evaluate whats needs to be done (mark special tokens)

        for(int finder = 0; finder<30;)
        {

            if(strcmp(evaluation[finder],"<")==0)
            {
                in = finder;

            }
            else if(strcmp(evaluation[finder],">")==0)
            {
                trunk =finder;
            ...
            if(evaluation[finder+1] == NULL || evaluation[finder] == "\0")
            {
                break;
            }
        ...
        for(int i=0; i<30;)
        {
            ...
            if(strcmp(evaluation[0],builtIn[0])==0)
            {
                ...
                for(int j=i+1; j<30;) // this sections accounts for spaces that might be in the path name doesn't account for 2 spaces
                {
                    ...
                    strcat(space," "); // if path has a space in it
                    j=j+1;
                }
                //printf("passing %s\n",space);
                cd(space);
            ...
            else if(strcmp(evaluation[0],builtIn[2])==0 && app == 0 && trunk == 0)
            {
                dir();
                break;
            }
            else if(strcmp(evaluation[0],builtIn[3])==0 && app == 0 && trunk == 0 )
            {
                environn(envp);
                break;
            }
            else if(strcmp(evaluation[0],builtIn[4])==0 && app == 0 && trunk == 0)
            {
                char echoo[15]="";
                for(int j=i+1; j<30;) // this sections accounts for spaces that might be in the path name doesn't account for 2 spaces
                {
            ...
            else if(strcmp(evaluation[0],builtIn[5])==0 && app == 0 && trunk == 0)
            {
                help();
                break;
            }
...
//-------------------------------------------------------------------------------------- end of built in functions
            else if(strcmp(evaluation[i],"<") == 0)
            {
                ...
                args = args - 1; // account for my arg counter count everything that isn't a token
            }
            else if(strcmp(evaluation[i],">")==0)
            {
                if(app != 0 || pipee != 0)
...
/*
        for(int see =0; see<10;) // let me see how commands were broken down
        {
            printf("%d-%s\n",see,evaluation[see]);
            see = see +1;
        }
...
// now we find out if arguments need to be passed
...
//-------------------------------------------------------------- arguments set up
        ...
        if(child == 0)
        {
            ...
            else if(in != 0)  // i want the child to asses all these statements
            {
                close(0); // close stdin
...
// child knows what to do now and arguments are passed if needed
            ...
            return 0;// stops child running its own shell with any set up redirection
...
// in parent
            if(pipee != 0)
            {
// create second child to do 2nd pipe command so parent is not overlaid
                int child4pipe = fork();
                if(child4pipe <0)
                {
                    printf("error");
                ...
                else
                {
                    ...
                    wait(NULL);// wait for new child
                }
                wait(NULL);
            }

            else if(bground == 0)  //wait if no background call
            {
                wait(NULL); // wait if pipee ==0
